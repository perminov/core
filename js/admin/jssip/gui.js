window.GUI = {
  ...
  createSession : function(display_name, uri) {
    var session_div = $('<div class="session">\
     \
      <div class="close"></div> \
      <div class="container"> \
        <div class="peer"> \
  ...
  setCallSessionStatus : function(session, status, description) {
    ...
    $('#startCall').unbind("click");
	$('.dtmfclick').unbind("click");
	$("#endCall").unbind("click");
    ...
    switch(status) {
      ...
      case "answered":
       ...
       //     sound_file = $(this).text();
          //soundPlayer.setAttribute("src", "sounds/dialpad/" + sound_file + ".ogg");
          //soundPlayer.play();
		session.call.sendDTMF(this.value);
        });
      ...
      case "terminated":
		...
		log("Разговор завершен");
		document.getElementById('player_symbol').pause();
        // Hide DTMF box.
		$('#startCall').css("display", "none"); 
		$('#startCall1').css("display", "block"); 
        dtmf_box.hide();
        break;
      ...
      case "incoming":
		...
		//alert('incoming1');
		var display_name;
		display_name = session.call.remote_identity.display_name || session.call.remote_identity.uri.user;
		log("Входящий вызов от " + display_name);
		document.getElementById('player_symbol').src = "/media/sounds/ringing.wav";
		document.getElementById('player_symbol').play();
		$('#startCall1').css("display", "none"); 
