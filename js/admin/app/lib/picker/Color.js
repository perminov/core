Ext.define('Ext.lib.picker.Color', {
	...
	afterRender: function(component) {
		var me = this;
		me.callParent(arguments);
		if (me.value) me.setColor(me.value);
	},
	...
	initEvents: function() {
		...
		me.down('#cSelect').on('click', me.selectClick, me);
	},
	...
	moveHuePicker: function(y) {
        ...
        y = Math.max(y, 0);
        y = Math.min(y, 181);
        hsv.h = Math.min(Math.round(360 / 181 * (181 - y)), 359);
        y = Math.max(y, 1);
	...
	moveRgbPicker: function(x, y) {
        ...
        x = Math.min(Math.max(x, 0), 181);
        y = Math.min(Math.max(y, 0), 181);

		hsv.s = me.getSaturation(x);
		hsv.v = me.getVal(y);

        x = Math.min(Math.max(x, 1), 180);
        y = Math.min(Math.max(y, 1), 180);
	...
	updateColor: function() {
		...
		if (me.updateMode != 'click') {
            ...
            var x = Math.max(Math.min(me.getSPos(me.down('#iSat').getValue() / 100), 180), 1);
            var y = Math.max(Math.min(me.getVPos(me.down('#iVal').getValue() / 100), 180), 1);

            me.down('#rgbPicker').getEl().moveTo(cRgb.getLeft() + x - 7, cHue.getTop() + y - 7, true);
            me.updateRgbPicker(hsv.h);
		}
	},

	setButtonColor: function(id, rgb) {
		var me = this, dq = Ext.DomQuery;
