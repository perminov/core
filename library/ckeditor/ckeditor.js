...
(function () {
    ...
    e.extend(h.prototype, {
        ...
        scrollIntoParent: function (i, j, k) {
            !i && (i = this.getWindow());
            var l = i.getDocument(),
                m = l.$.compatMode == 'BackCompat';
            if (i instanceof d.window) i = m ? l.getBody() : l.getDocumentElement();

            function n(z, A) {
                if (/body|html/.test(i.getName())) i.getWindow().$.scrollBy(z, A);
                else {
                    i.$.scrollLeft += z;
                    i.$.scrollTop += A;
                }
            ...
            var q = i.getWindow(),
                ...
                x, y;
            x = {
                x: r.x - p(this, 'left') - s.x || 0,
                y: r.y - p(this, 'top') - s.y || 0
            };
            y = {
                x: r.x + u + p(this, 'right') - (s.x + w) || 0,
                y: r.y + t + p(this, 'bottom') - (s.y + v) || 0
            };
    ...
    (function () {
        ...
        a.htmlParser.fragment.fromHtml = function (q, r, s) {
            ...
            t.onText = function (D) {
                ...
                var E = x.name,
                    F = E ? f[E] || (x._.isBlockLike ? f.div : f.span) : o;
                if (!y && !F['#'] && E in l) {
    ...
    (function () {
        function l(s, t) {
            ...
            var v, w = u.startContainer,
                x = u.endContainer,
                y = u.startOffset,
                z = u.endOffset,
                A, B = this.guard,
                C = this.type,
                D = s ? 'getPreviousSourceNode' : 'getNextSourceNode';
            if (!s && !this._.guardLTR) {
                var E = x.type == 1 ? x : x.getParent(),
                    F = x.type == 1 ? x.getChild(z) : x.getNext();
                this._.guardLTR = function (J, K) {
                    return (!K || !E.equals(J)) && (!F || !J.equals(F)) && (J.type != 1 || !K || J.getName() != 'body');
                };
            }
            if (s && !this._.guardRTL) {
                var G = w.type == 1 ? w : w.getParent(),
                    H = w.type == 1 ? y ? w.getChild(y - 1) : null : w.getPrevious();
            ...
            if (this.current) v = this.current[D](false, C, A);
            else {
                if (s) {
                    v = x;
                    if (v.type == 1) if (z > 0) v = v.getChild(z - 1);
                    else v = A(v, true) === false ? null : v.getPreviousSourceNode(true, C, A);
                } else {
                    v = w;
        ...
        d.walker.bogus = function (s, t) {
            ...
            return function (v) {
                var w = v.getParent(),
                    x = !c ? v.is && v.is('br') : v.getText && o.test(v.getText());
    ...
    (function () {
        var l = function (u) {
            ...
            u.optimizeBookmark();
            var y = u.startContainer,
                z = u.endContainer,
                A = u.startOffset,
                B = u.endOffset,
                C, D;
            if (z.type == 3) z = z.split(B);
            else if (z.getChildCount() > 0) if (B >= z.getChildCount()) {
            ...
            for (var P = G; P < F.length; P++) {
                ...
                if (!E[P] || K.$.parentNode != E[P].$.parentNode) {
                    ...
                    while (M) {
                        ...
                        N = M.getPrevious();
                        if (v == 2) J.$.insertBefore(M.$.cloneNode(true), J.$.firstChild);
                        else {
                            M.remove();
        ...
        function o(u) {
            var v = false,
                w = d.walker.bookmark(true);
            return function (x) {
                if (w(x)) return true;
                if (x.type == 3) {
                    if (x.hasAscendant('pre') || e.trim(x.getText()).length) return false;
                } else if (x.type == 1) if (!n[x.getName()]) if (!u && !c && x.getName() == 'br' && !v) v = true;
                else return false;
                return true;
            };
        };
        ...
        var r = new d.walker.whitespaces(),
            s = new d.walker.bookmark();
        ...
        d.range.prototype = {
            ...
            createBookmark2: function (u) {
                ...
                return {
                    start: v.getAddress(u),
                    end: z ? null : w.getAddress(u),
                    startOffset: x,
                    endOffset: y,
                    normalized: u,
                    collapsed: z,
            ...
            moveToBookmark: function (u) {
                var C = this;
                if (u.is2) {
                    var v = C.document.getByAddress(u.start, u.normalized),
                        w = u.startOffset,
                        x = u.end && C.document.getByAddress(u.end, u.normalized),
                        y = u.endOffset;
                    C.setStart(v, w);
                    if (x) C.setEnd(x, y);
                    else C.collapse(true);
                } else {
                    var z = u.serializable,
                        A = z ? C.document.getById(u.startNode) : u.startNode,
                        B = z ? C.document.getById(u.endNode) : u.endNode;
                    C.setStartBefore(A);
                    A.remove();
                    if (B) {
                        C.setEndBefore(B);
            ...
            getBoundaryNodes: function () {
                ...
                if (u.type == 1) {
                    y = u.getChildCount();
                    if (y > w) u = u.getChild(w);
                    else if (y < 1) u = u.getPreviousSourceNode();
                    else {
                        u = u.$;
                ...
                if (v.type == 1) {
                    y = v.getChildCount();
                    if (y > x) v = v.getChild(x).getPreviousSourceNode(true);
                    else if (y < 1) v = v.getPreviousSourceNode();
                    else {
                        v = v.$;
            ...
            getCommonAncestor: function (u, v) {
                var z = this;
                var w = z.startContainer,
                    x = z.endContainer,
                    y;
                if (w.equals(x)) {
                    if (u && w.type == 1 && z.startOffset == z.endOffset - 1) y = w.getChild(z.startOffset);
                    else y = w;
            ...
            optimize: function () {
                var w = this;
                var u = w.startContainer,
                    v = w.startOffset;
                if (u.type != 1) if (!v) w.setStartBefore(u);
                else if (v >= u.getLength()) w.setStartAfter(u);
                u = w.endContainer;
                v = w.endOffset;
                if (u.type != 1) if (!v) w.setEndBefore(u);
                else if (v >= u.getLength()) w.setEndAfter(u);
            },
            ...
            enlarge: function (u, v) {
                switch (u) {
                    case 1:
                        if (this.collapsed) return;
                        var w = this.getCommonAncestor(),
                            x = this.document.getBody(),
                            y, z, A, B, C, D = false,
                            E, F, G = this.startContainer,
                            H = this.startOffset;
                        if (G.type == 3) {
                            if (H) {
                        ...
                        while (A || B) {
                            ...
                            while (B) {
                                ...
                                if (E) if (D) {
                                    if (C) y = A;
                                    else if (A) this.setStartBefore(A);
                                } else D = true;
                                if (B) {
                                    var L = B.getPrevious();
                                    if (!A && !L) {
                    ...
                    case 3:
                        ...
                        var N = new d.walker(M),
                            O, P, Q = d.walker.blockBoundary(u == 3 ? {
                                br: 1
                            } : null),
                            R = function (X) {
                                var Y = Q(X);
            ...
            shrink: function (u, v) {
                if (!this.collapsed) {
                    ...
                    var w = this.clone(),
                        ...
                        C = 1,
                        D = 1;
                    if (x && x.type == 3) if (!z) w.setStartBefore(x);
                    else if (z >= x.getLength()) w.setStartAfter(x);
                    else {
                        w.setStartBefore(x);
                        C = 0;
                    }
                    if (y && y.type == 3) if (!A) w.setEndBefore(y);
                    else if (A >= y.getLength()) w.setEndAfter(y);
                    else {
                        w.setEndAfter(y);
                        D = 0;
                    }
                    var E = new d.walker(w),
                        F = d.walker.bookmark();
            ...
            setStartAt: function (u, v) {
                ...
                switch (v) {
                    case 1:
                        ...
                        break;
                    case 2:
                        if (u.type == 3) w.setStart(u, u.getLength());
                        else w.setStart(u, u.getChildCount());
                        break;
                    case 3:
                        w.setStartBefore(u);
                        break;
            ...
            setEndAt: function (u, v) {
                ...
                switch (v) {
                    case 1:
                        ...
                        break;
                    case 2:
                        if (u.type == 3) w.setEnd(u, u.getLength());
                        else w.setEnd(u, u.getChildCount());
                        break;
                    case 3:
                        w.setEndBefore(u);
                        break;
            ...
            fixBlock: function (u, v) {
                var y = this;
                var w = y.createBookmark(),
                    x = y.document.createElement(v);
                y.collapse(u);
                y.enlarge(2);
            ...
            splitBlock: function (u) {
                var E = this;
                var v = new d.elementPath(E.startContainer),
                    w = new d.elementPath(E.endContainer),
                    x = v.blockLimit,
                    y = w.blockLimit,
                    z = v.block,
                    A = w.block,
                ...
                var C = z && E.checkStartOfBlock(),
                    D = A && E.checkEndOfBlock();
                E.deleteContents();
            ...
            checkStartOfBlock: function () {
                ...
                A.trim();
                var x = new d.elementPath(A.startContainer),
                    y = A.clone();
                y.collapse(true);
            ...
            checkEndOfBlock: function () {
                ...
                A.trim();
                var x = new d.elementPath(A.endContainer),
                    y = A.clone();
                y.collapse(false);
            ...
            checkReadOnly: (function () {
                function u(v, w) {
                    while (v) {
                        if (v.type == 1) if (v.getAttribute('contentEditable') == 'false' && !v.data('cke-editable')) return 0;
            ...
            function r(t, u, v) {
                var w = [],
                    ...
                    y = e.addFunction(function (E, F) {
                        ...
                        t.hide(false);
                        m.fire('saveSnapshot');
                        new a.style(n['colorButton_' + F + 'Style'], {
                            color: 'inherit'
                        }).remove(m.document);
                        if (E) {
    ...
    (function () {
        j.add('image', {
            init: function (o) {
                ...
                o.ui.addButton('Image', {
                    label: o.lang.common.image,
                    command: p
                });
                ...
                if (o.addMenuItems) o.addMenuItems({
                    image: {
                        label: o.lang.image.menu,
                        command: 'image',
                        group: 'image'
                    }
                });
    ...
    (function () {
        function m(q, r) {
            r = r === undefined || r;
            var s;
            if (r) s = q.getComputedStyle('text-align');
            else {
        ...
        function p(q) {
            ...
            while (u = t.next()) {
                if (u.type == 1) {
                    ...
                    var w = 'text-align',
                        x = u.getStyle(w);
                    if (x == 'left') u.setStyle(w, 'right');
                    else if (x == 'right') u.setStyle(w, 'left');
                }
            }
        };
        o.prototype = {
            exec: function (q) {
                ...
                var r = q.getSelection(),
                    s = q.config.enterMode;
                if (!r) return;
                var t = r.createBookmarks(),
                    u = r.getRanges(true),
                    v = C.cssClassName,
                    w, x, y = q.config.useComputedState;
                y = y === undefined || y;
                for (var z = u.length - 1; z >= 0; z--) {
                    ...
                    while (x = w.getNextParagraph(s == 1 ? 'p' : 'div')) {
                        ...
                        var A = v && (x.$.className = e.ltrim(x.$.className.replace(C.cssClassRegex, ''))),
                            B = C.state == 2 && (!y || m(x, true) != C.value);
                        if (v) {
                            if (B) x.addClass(v);
                            else if (!A) x.removeAttribute('class');
        ...
        j.add('justify', {
            init: function (q) {
                var r = new o(q, 'justifyleft', 'left'),
                    s = new o(q, 'justifycenter', 'center'),
                    t = new o(q, 'justifyright', 'right'),
                    u = new o(q, 'justifyblock', 'justify');
                q.addCommand('justifyleft', r);
                q.addCommand('justifycenter', s);
                q.addCommand('justifyright', t);
                q.addCommand('justifyblock', u);
                q.ui.addButton('JustifyLeft', {
                    label: q.lang.justify.left,
                    command: 'justifyleft'
                });
                q.ui.addButton('JustifyCenter', {
                    label: q.lang.justify.center,
                    command: 'justifycenter'
                });
                q.ui.addButton('JustifyRight', {
                    label: q.lang.justify.right,
                    command: 'justifyright'
                });
                q.ui.addButton('JustifyBlock', {
                    label: q.lang.justify.block,
    ...
    (function () {
        ...
        j.list = {
            listToArray: function (L, M, N, O, P) {
                ...
                for (var Q = 0, R = L.getChildCount(); Q < R; Q++) {
                    ...
                    for (var U = 0, V = S.getChildCount(), W; U < V; U++) {
                        W = S.getChild(U);
                        if (W.type == 1 && m[W.getName()]) j.list.listToArray(W, M, N, O + 1, T.grandparent);
                        else T.contents.push(W);
                    }
                }
                return N;
            },
            arrayToList: function (L, M, N, O, P) {
                ...
                var Q, R = L[N].parent.getDocument(),
                    S = new d.documentFragment(R),
                    T = null,
                    U = N,
                    V = Math.max(L[N].indent, 0),
                    W = null,
                    X, Y, Z = O == 1 ? 'p' : 'div';
                while (1) {
                    var aa = L[U];
                    ...
                    if (aa.indent == V) {
                        ...
                        var ab = L[U - 1].element.getDirection(1),
                            ac = j.list.arrayToList(L, null, U, O, ab != X ? X : null);
                        if (!W.getChildCount() && c && !(R.$.documentMode > 7)) W.append(R.createText('\xa0'));
                        W.append(ac.listNode);
                        U = ac.nextIndex;
                    } else if (aa.indent == -1 && !N && aa.grandparent) {
                        if (m[aa.grandparent.getName()]) W = aa.element.clone(false, true);
                        else W = new d.documentFragment(R);
                        var ad = aa.grandparent.getDirection(1) != X,
                            ae = W.type == 11 && O != 2,
                            af = aa.element,
                            ag = af.getAttribute('class'),
                            ah = af.getAttribute('style'),
                            ai, aj = aa.contents.length;
                        for (Q = 0; Q < aj; Q++) {
                            ai = aa.contents[Q];
        ...
        function u(L, M, N, O) {
            var P = j.list.listToArray(M.root, N),
                Q = [];
            ...
            var T = M.root,
                U = T.getDocument().createElement(this.type);
            T.copyAttributes(U, {
                start: 1,
                type: 1
            });
            ...
            var W = j.list.arrayToList(P, N, null, L.config.enterMode),
                X, Y = W.listNode.getChildCount();
        ...
        function w(L, M, N) {
            ...
            var U = L.config.useComputedState,
                V, W;
            U = U === undefined || U;
            ...
            if (Q.length < 1) return;
            var aa = Q[Q.length - 1].getNext(),
                ab = P.createElement(this.type);
            N.push(ab);
            var ac, ad;
            while (Q.length) {
                ac = Q.shift();
                ad = P.createElement('li');
                if (ac.is('pre') || v.test(ac.getName())) ac.appendTo(ad);
                else {
            ...
            if (aa) ab.insertBefore(aa);
            else ab.appendTo(S);
        };

        function x(L, M, N) {
            var O = j.list.listToArray(M.root, N),
                P = [];
            ...
            for (Q = S + 1; Q < O.length; Q++) {
                if (O[Q].indent > O[Q - 1].indent + 1) {
                    var U = O[Q - 1].indent + 1 - O[Q].indent,
                        V = O[Q].indent;
            ...
            var W = j.list.arrayToList(O, N, null, L.config.enterMode, M.root.getAttribute('dir')),
                X = W.listNode,
                Y, Z;

            function aa(ab) {
                if ((Y = X[ab ? 'getFirst' : 'getLast']()) && !(Y.is && Y.isBlockBoundary()) && (Z = M.root[ab ? 'getPrevious' : 'getNext'](d.walker.whitespaces(true))) && !(Z.is && Z.isBlockBoundary({
                    br: 1
                }))) L.document.createElement('br')[ab ? 'insertBefore' : 'insertAfter'](Y);
            };
            aa(true);
            aa();
            X.replace(M.root);
            ...
            exec: function (L) {
                ...
                if (this.state == 2) {
                    ...
                    if (!Q.getFirst(q)) {
                        ...
                        var R = P.length == 1 && P[0],
                            S = R && R.getEnclosedNode();
                        if (S && S.is && this.type == S.getName()) this.setState(1);
                    }
                }
                var T = O.createBookmarks(true),
                    U = [],
                    V = {}, W = P.createIterator(),
                    X = 0;
                while ((R = W.getNextRange()) && ++X) {
                    var Y = R.getBoundaryNodes(),
                        Z = Y.startNode,
                        aa = Y.endNode;
                    if (Z.type == 1 && Z.getName() == 'td') R.setStartAt(Y.startNode, 1);
                    if (aa.type == 1 && aa.getName() == 'td') R.setEndAt(Y.endNode, 2);
                    var ab = R.createIterator(),
                        ac;
                    ab.forceBrBreak = this.state == 2;
                    while (ac = ab.getNextParagraph()) {
                        if (ac.getCustomData('list_block')) continue;
                        else h.setMarker(V, ac, 'list_block', 1);
                        var ad = new d.elementPath(ac),
                            ae = ad.elements,
                            af = ae.length,
                            ag = null,
                            ah = 0,
                        ...
                        if (ah) continue;
                        var am = ai;
                        if (am.getCustomData('list_group_object_' + X)) am.getCustomData('list_group_object_' + X).contents.push(ac);
                        else {
                            al = {
                                root: am,
                                contents: [ac]
                ...
                while (U.length > 0) {
                    al = U.shift();
                    if (this.state == 2) {
                        if (m[al.root.getName()]) u.call(this, L, al, V, an);
                        else w.call(this, L, al, an);
        ...
        function E(L) {
            return function (M) {
                ...
                if (Q && (Q.name && Q.name == 'br' || Q.value && (R = Q.value.match(C)))) {
                    var S = Q;
                    if (!(R && R.index) && S == N[0]) N[0] = L || c ? new a.htmlParser.text('\xa0') : new a.htmlParser.element('br', {});
                    else if (S.name == 'br') N.splice(O - 1, 1);
                    else S.value = S.value.replace(C, '');
                }
            };
        };
        var F = {
        ...
        function J(L, M, N) {
            ...
            M.trim(false, true);
            var P = new d.elementPath(M.startContainer),
                Q = P.lastElement.getAscendant('li', 1),
                R = P.block.getBogus();
            R && R.remove();
            var S = O.getLast();
            if (S && S.type == 1 && S.is('br')) S.remove();
            var T = M.startContainer.getChild(M.startOffset);
            if (T) O.insertBefore(T);
            else M.startContainer.append(O);
            var U = new d.elementPath(N.startContainer),
                V = N.startContainer.getAscendant('li', 1);
            if (V) {
                var W = K(V);
        ...
        j.add('list', {
            init: function (L) {
                var M = L.addCommand('numberedlist', new y('numberedlist', 'ol')),
                    N = L.addCommand('bulletedlist', new y('bulletedlist', 'ul'));
                L.ui.addButton('NumberedList', {
                    label: L.lang.numberedlist,
                    command: 'numberedlist'
                });
                L.ui.addButton('BulletedList', {
                    label: L.lang.bulletedlist,
                ...
                if (b.ie8Compat) L.on('key', function (O) {
                    ...
                    if (L.mode == 'wysiwyg' && P in {
                        ...
                        var S = P == 8,
                            T = L.document.getBody(),
                            U = new d.walker(R.clone());
        ...
        j.add('scayt', {
            ...
            init: function (t) {
                ...
                if (t.contextMenu && t.addMenuItems) t.contextMenu.addListener(function (E, F) {
                    ...
                    if (!I) return null;
                    var J = G.getLang(),
                        K = {}, L = window.scayt.getSuggestion(I, J);
    ...
    (function () {
        ...
        o.prototype = {
            ...
            restoreImage: function (s) {
                ...
                if (s.bookmarks) u.selectBookmarks(s.bookmarks);
                else if (c) {
        ...
        j.add('wysiwygarea', {
            ...
            init: function (C) {
                var D = C.config.enterMode != 2 && C.config.autoParagraph !== false ? C.config.enterMode == 3 ? 'div' : 'p' : false,
                    ...
                    F = a.document.getWindow(),
                    G;
                C.on('editingBlockReady', function () {
                    var L, M, N, O, P, Q, R, S = b.isCustomDomain(),
                        T = function (W) {
                            if (M) M.remove();
                    ...
                    function V(W) {
                        ...
                        var X = W.document,
                            Y = X.body,
                            Z = X.getElementById('cke_actscrpt');
                        ...
                        if (c) {
                            ...
                            if (b.gecko && b.version >= 10900 || b.opera) X.$.body.contentEditable = aa;
                        ...
                        ab.on('focus', function () {
                            var af = C.document;
                            if (aa && b.gecko && b.version >= 10900) K();
                    ...
                    C.addMode('wysiwyg', {
                        load: function (W, X, Y) {
                            ...
                            C.mayBeDirty = true;
                            Q = true;
                            if (Y) this.loadSnapshotData(X);
                            else this.loadData(X);
                        },
                        loadData: function (W) {
                            N = true;
                            C._.dataStore = {
                            ...
                            if (Y) {
                                ...
                                if (!/<head[\s|>]/.test(W)) W = W.replace(/<html[^>]*>/, '$&<head><title></title></head>');
                                else if (!/<title[\s|>]/.test(W)) W = W.replace(/<head[^>]*>/, '$&<title></title>');
                        ...
                        getData: function () {
                            var W = C.config,
                                X = W.fullPage,
                                Y = X && C.docType,
                                Z = X && C.xmlDeclaration,
                                aa = M.getFrameDocument(),
                                ab = X ? aa.getDocumentElement().getOuterHtml() : aa.getBody().getHtml();
    ...
    (function () {
        ...
        a.dialog = function (S, T) {
            ...
            function ak(ar) {
                ...
                var au = (at + ar + as.length) % as.length,
                    av = au;
            ...
            k.dialog = {
                uiElement: function (S, T, U, V, W, X, Y) {
                    ...
                    var Z = (V.call ? V(T) : V) || 'div',
                        aa = ['<', Z, ' '],
                        ab = (W && W.call ? W(T) : W) || {}, ac = (X && X.call ? X(T) : X) || {}, ad = (Y && Y.call ? Y.call(this, S, T) : Y) || '',
            ...
            k.panelButton = e.createClass({
                ...
                proto: {
                    createPanel: function (n) {
                        var o = this._;
                        if (o.panel) return;
                        var p = this._.panelDefinition || {}, q = this._.panelDefinition.block,
                    ...
                    e.setTimeout(function () {
                        var E = e.bind(function () {
                            ...
                            var J = u.element,
                                ...
                                M = K.getViewPaneSize(),
                                N = L.width || L.right - L.left,
                                O = L.height || L.bottom - L.top,
                                P = A ? L.right : M.width - L.left,
                                Q = A ? M.width - L.right : L.left;
                            if (A) {
                                if (P < N) if (Q > N) B += N;
                                else if (M.width > N) B -= L.left;
                                else B = B - L.right + M.width;
                            } else if (P < N) if (Q > N) B -= N;
                            else if (M.width > N) B = B - L.right + M.width;
                            else B -= L.left;
                            var R = M.height - L.top,
                                S = L.top;
                            if (R < O) if (S > O) C -= O;
                            else if (M.height > O) C = C - L.bottom + M.height;
                            else C -= L.top;
                            if (c) {
                                var T = new h(w.$.offsetParent),
                                    U = T;
                                if (U.getName() == 'html') U = U.getDocument().getBody();
                                if (U.getComputedStyle('direction') == 'rtl') if (b.ie8Compat) B -= w.getDocument().getDocumentElement().$.scrollLeft * 2;
                                else B -= T.$.scrollWidth - T.$.clientWidth;
                            }
                            var V = w.getFirst(),
                                W;
    ...
    j.add('menu', {
        beforeInit: function (m) {
            var n = m.config.menu_groups.split(','),
                o = m._.menuGroups = {}, p = m._.menuItems = {};
    ...
    (function () {
        function m() {
            ...
            try {
                ...
                var A = z.getStartElement(),
                    B = new d.elementPath(A);
        ...
        var s = {
            ...
            exec: function (z) {
                switch (z.mode) {
                    ...
                    case 'source':
                        var A = z.textarea.$;
                        if (c) A.createTextRange().execCommand('SelectAll');
                        else {
                            A.selectionStart = 0;
        ...
        function v(z) {
            var A = z && u(z);
            if (A) if (A.getCustomData('ready')) w(z);
            else A.setCustomData('ready', 1);
        };

        function w(z) {
            var A = z && z.removeCustomData('cke-fillingChar');
            if (A) {
                var B, C = z.getSelection().getNative(),
                    D = C && C.type != 'None' && C.getRangeAt(0);
                if (A.getLength() > 1 && D && D.intersectsNode(A.$)) {
                    B = [C.anchorOffset, C.focusOffset];
                    var E = C.anchorNode == A.$ && C.anchorOffset > 0,
                        F = C.focusNode == A.$ && C.focusOffset > 0;
                    E && B[0]--;
                    F && B[1]--;
                    x(C) && B.unshift(B.pop());
                }
        ...
        j.add('selection', {
            init: function (z) {
                    ...
                    if (c) {
                        ...
                        function M(N) {
                            if (I) {
                                ...
                                var R;
                                if (Q && Q.type && Q.type != 'Control' && (R = Q.createRange()) && (R = R.parentElement()) && (R = R.nodeName) && R.toLowerCase() in {
                                    input: 1,
                                    textarea: 1
                                }) return;
                                H = Q && P.getRanges()[0];
                ...
                z.ui.addButton('SelectAll', {
                    label: z.lang.selectAll,
                    command: 'selectAll'
                });
                z.selectionChange = p;
        ...
        d.selection.prototype = {
            ...
            getRanges: (function () {
                var z = c ? (function () {
                    ...
                    var B = function (C, D) {
                        ...
                        while (K <= L) {
                            ...
                            else if (b.ie9Compat && H.tagName == 'BR') {
                ...
                return function (A) {
                    var B = this._.cache;
                    if (B.ranges && !A) return B.ranges;
                    else if (!B.ranges) B.ranges = new d.rangeList(z.call(this));
                    if (A) {
                        var C = B.ranges;
                        for (var D = 0; D < C.length; D++) {
                            var E = C[D],
                                F = E.getCommonAncestor();
                            ...
                            var H = E.startContainer,
                                ...
                                K = E.endOffset,
                                L = E.clone();
                            if (H && H.type == 3) if (J >= H.getLength()) L.setStartAfter(H);
                            else L.setStartBefore(H);
                            if (I && I.type == 3) if (!K) L.setEndBefore(I);
                            else L.setEndAfter(I);
                            ...
                            if (!C.collapsed) {
                                ...
                                while (1) {
                                    var D = C.startContainer,
                                        E = C.startOffset;
                                    if (E == (D.getChildCount ? D.getChildCount() : D.getLength()) && !D.isBlockBoundary()) C.setStartAfter(D);
                                    else break;
                                }
                                A = C.startContainer;
                                if (A.type != 1) return A.getParent();
                                A = A.getChild(C.startOffset);
                                if (!A || A.type != 1) A = C.startContainer;
                                else {
                                    var F = A.getFirst();
                                    while (F && F.type == 1) {
                                        A = F;
            ...
            getSelectedElement: function () {
                ...
                var A = this,
                    B = e.tryThese(function () {
                        return A.getNative().createRange().item(0);
                    }, function () {
                        var C, D, E = A.getRanges()[0],
                            F = E.getCommonAncestor(1, 1),
                            G = {
                                table: 1,
                                ul: 1,
                                ol: 1,
                        ...
                        if (C) {
                            ...
                            var J = e.extend(G, f.$listItem, f.$tableContent),
                                K = new d.walker(I),
                                L = function (M, N) {
                                    return function (O, P) {
            ...
            unlock: function (z) {
                var E = this;
                var A = E.document,
                    B = A.getCustomData('cke_locked_selection');
                if (B) {
                    ...
                    if (z) {
                        ...
                        E.reset();
                        A.getBody().focus();
                        if (C) E.selectElement(C);
                        else E.selectRanges(D);
                    }
                }
                if (!B || !z) {
                    E.isLocked = 0;
            ...
            getCommonAncestor: function () {
                var z = this.getRanges(),
                    A = z[0].startContainer,
                    B = z[z.length - 1].endContainer;
    ...
    k.panel.prototype = {
        ...
        showBlock: function (m) {
            ...
            var n = r._.blocks,
                o = n[m],
                p = r._.currentBlock,
                q = !r.forceIFrame || c ? r._.holder : r.document.getById(r.id + '_frame');
            if (p) {
