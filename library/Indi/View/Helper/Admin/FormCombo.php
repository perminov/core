...
class Indi_View_Helper_Admin_FormCombo extends Indi_View_Helper_Abstract{
    ...
    public function formCombo($name, $tableName = null){
        ...
        if ($this->view->row->id) {
            $selected = $this->view->row->$name;
        } else {
            $selected = $field->defaultValue;
        }

        // Get initial set of combo options
        $comboDataRs = $this->view->row->getComboData($name, null, $selected);
...
<div class="combo-div">
    ...
    <?} else if ($field->storeRelationAbility == 'many') {?>
<style>
.combo-div .combo-multiple .combo-selected-item{
    margin-left: 1px;
...
.combo-div .combo-multiple .combo-selected-item .combo-selected-item-delete{
...
.combo-div .combo-multiple .combo-keyword{
    margin-left: 1px;
    padding-left: 0;
}
</style>
<div class="combo-div" style="position: relative; line-height: 15px;">
    <img style="display: none;" class="combo-trigger" id="<?=$name?>-trigger" src="/i/admin/trigger-system.png"/>
    <div class="combo-multiple">
        <?foreach($comboDataRs->selected as $selectedR){?><span class="combo-selected-item" selected-id="<?=$selectedR->$keyProperty?>"><?=Misc::usubstr($selectedR->title, 50)?><span class="combo-selected-item-delete"></span></span><?}
        ?><input style="border: 0px solid; margin-top: 1px;width: 30px; height: 15px;"  class="combo-keyword" type="text" id="<?=$name?>-keyword" lookup="<?=$name?>" value="<?=$selected['title']?>" no-lookup="<?=$params['noLookup']?>"/>
        <input type="hidden" id="<?=$name?>" value="<?=$selected['value']?>" name="<?=$name?>"<?=$attrs?>/>
        <span class="combo-info<?=$field->storeRelationAbility == 'many'?' multiple':''?>" id="<?=$name?>-info" page-top="0" page-btm="0" fetch-mode="no-keyword" page-top-reached="<?=$pageUpDisabled?>" page-btm-reached="false" satellite="<?=$satellite->alias?>" changed="false">
            <span class="combo-count" id="<?=$name?>-count"></span>
            <span class="combo-of"><?=COMBO_OF?></span>
            <span class="combo-found" id="<?=$name?>-found"></span>
        </span>
    ...
    <?}?>
<script>var comboOptions = comboOptions || {}; comboOptions["<?=$name?>"] = (<?=$options?>);</script>
        <?return ob_get_clean();
    }
}