...
/**
 * @see Indi_Session_Abstract
 */
require_once 'Indi/Session/Abstract.php';

/**
 * @see Indi_Session_Namespace
 */
require_once 'Indi/Session/Namespace.php';

/**
 * @see Indi_Session_SaveHandler_Interface
 */
require_once 'Indi/Session/SaveHandler/Interface.php';
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Indi_Session extends Indi_Session_Abstract
{
    /**
     * Whether or not Indi_Session is being used with unit tests
     *
     * @internal
     * @var bool
     */
    ...
    /**
     * List of options pertaining to Indi_Session that can be set by developers
     * using Indi_Session::setOptions(). This list intentionally duplicates
    ...
    /**
     * Whether the default options listed in Indi_Session::$_localOptions have been set
     *
     * @var bool
     */
     ...
     *
     * @var Indi_Session_SaveHandler_Interface
     */
    ...
    public static function setOptions(array $userOptions = array())
    {
        ...
        foreach ($userOptions as $userOptionName => $userOptionValue) {
            ...
            elseif (isset(self::$_localOptions[$userOptionName])) {
                ...
                /** @see Indi_Session_Exception */
                require_once 'Indi/Session/Exception.php';
                throw new Indi_Session_Exception("Unknown option: $userOptionName = $userOptionValue");
            }
        }
    }
     ...
     *
     * @param Indi_Session_SaveHandler_Interface $interface
     * @return void
     */
    public static function setSaveHandler(Indi_Session_SaveHandler_Interface $saveHandler)
    {
     ...
     *
     * @return Indi_Session_SaveHandler_Interface
     */
    ...
    public static function regenerateId()
    {
        if (!self::$_unitTestEnabled && headers_sent($filename, $linenum)) {
            /** @see Indi_Session_Exception */
            require_once 'Indi/Session/Exception.php';
            throw new Indi_Session_Exception("You must call " . __CLASS__ . '::' . __FUNCTION__ .
    ...
    public static function start($options = false)
    {
        if (self::$_sessionStarted && self::$_destroyed) {
            require_once 'Indi/Session/Exception.php';
            throw new Indi_Session_Exception('The session was explicitly destroyed during this request, attempting to re-start is not allowed.');
        }
        ...
        // In strict mode, do not allow auto-starting Indi_Session, such as via "new Indi_Session_Namespace()"
        if (self::$_strict && $options === true) {
            /** @see Indi_Session_Exception */
            require_once 'Indi/Session/Exception.php';
            throw new Indi_Session_Exception('You must explicitly start the session with Indi_Session::start() when session options are set to strict.');
        }
        ...
        if (!self::$_unitTestEnabled && headers_sent($filename, $linenum)) {
            /** @see Indi_Session_Exception */
            require_once 'Indi/Session/Exception.php';
            throw new Indi_Session_Exception("Session must be started before any output has been sent to the browser;"
        ...
        if (!self::$_unitTestEnabled && defined('SID')) {
            /** @see Indi_Session_Exception */
            require_once 'Indi/Session/Exception.php';
            throw new Indi_Session_Exception('session has already been started by session.auto-start or session_start()');
        }
        ...
        if (!self::$_unitTestEnabled) {
            ...
            if (self::$_throwStartupExceptions) {
                require_once 'Indi/Session/Exception.php';
                set_error_handler(array('Indi_Session_Exception', 'handleSessionStartError'), $errorLevel);
            }
            ...
            if (!$startedCleanly || Indi_Session_Exception::$sessionStartError != null) {
                if (self::$_throwStartupExceptions) {
                    set_error_handler(array('Indi_Session_Exception', 'handleSilentWriteClose'), $errorLevel);
                }
                session_write_close();
                if (self::$_throwStartupExceptions) {
                    restore_error_handler();
                    throw new Indi_Session_Exception(__CLASS__ . '::' . __FUNCTION__ . '() - ' . Indi_Session_Exception::$sessionStartError);
                }
            }
        }
     ...
     * has been called during this request by Indi_Session.
     *
     * @return bool
     */
    ...
    public static function setId($id)
    {
        if (!self::$_unitTestEnabled && defined('SID')) {
            /** @see Indi_Session_Exception */
            require_once 'Indi/Session/Exception.php';
            throw new Indi_Session_Exception('The session has already been started.  The session id must be set first.');
        }
        ...
        if (!is_string($id) || $id === '') {
            /** @see Indi_Session_Exception */
            require_once 'Indi/Session/Exception.php';
            throw new Indi_Session_Exception('You must provide a non-empty string as a session identifier.');
        }
     ...
     *
     * @param Indi_Session_Validator_Interface $validator
     * @return void
     */
    public static function registerValidator(Indi_Session_Validator_Interface $validator)
    {
        $validator->setup();
    }
     ...
     *
     * @param bool $readonly - OPTIONAL remove write access (i.e. throw error if Indi_Session's attempt writes)
     * @return void
     */
    ...
    private static function _processValidators()
    {
        foreach ($_SESSION['__ZF']['VALID'] as $validator_name => $valid_data) {
            if (!class_exists($validator_name)) {
                ...
                Indi_Loader::loadClass($validator_name);
            }
            $validator = new $validator_name;
            if ($validator->validate() === false) {
                /** @see Indi_Session_Exception */
                require_once 'Indi/Session/Exception.php';
                throw new Indi_Session_Exception("This session is not valid according to {$validator_name}.");
            }
        }
    }
    ...
    public static function namespaceUnset($namespace)
    {
        ...
        Indi_Session_Namespace::resetSingleInstance($namespace);
    }


    /**
     * namespaceGet() - get all variables in a namespace
     * Deprecated: Use getIterator() in Indi_Session_Namespace.
     *
     * @param string $namespace
     * @return array
     */
    ...
    public static function getIterator()
    {
        if (parent::$_readable === false) {
            /** @see Indi_Session_Exception */
            require_once 'Indi/Session/Exception.php';
            throw new Indi_Session_Exception(parent::_THROW_NOT_READABLE_MSG);
        }
