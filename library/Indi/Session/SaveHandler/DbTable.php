 ...
 * @see Indi_Session
 */
require_once 'Indi/Session.php';

/**
 * @see Indi_Db_Table_Abstract
 */
require_once 'Indi/Db/Table/Abstract.php';

/**
 * @see Indi_Db_Table_Row_Abstract
 */
require_once 'Indi/Db/Table/Row/Abstract.php';

/**
 * @see Indi_Config
 */
require_once 'Indi/Config.php';

/**
 * Indi_Session_SaveHandler_DbTable
 *
 * @category   Indi
 * @package    Indi_Session
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Indi_Session_SaveHandler_DbTable
    extends Indi_Db_Table_Abstract
    implements Indi_Session_SaveHandler_Interface
{
     ...
     * Constructor
     *
     * $config is an instance of Indi_Config or an array of key/value pairs containing configuration options for
     * Indi_Session_SaveHandler_DbTable and Indi_Db_Table_Abstract. These are the configuration options for
     * Indi_Session_SaveHandler_DbTable:
     *
     ...
     *
     * @param  Indi_Config|array $config      User-provided configuration
     * @return void
     * @throws Indi_Session_SaveHandler_Exception
     */
    public function __construct($config)
    {
        if ($config instanceof Indi_Config) {
            $config = $config->toArray();
        } else if (!is_array($config)) {
            /**
             * @see Indi_Session_SaveHandler_Exception
             */
            require_once 'Indi/Session/SaveHandler/Exception.php';

            throw new Indi_Session_SaveHandler_Exception(
                '$config must be an instance of Indi_Config or array of key/value pairs containing '
              . 'configuration options for Indi_Session_SaveHandler_DbTable and Indi_Db_Table_Abstract.');
        }
     ...
     * @return Indi_Session_SaveHandler_DbTable
     */
    ...
    protected function _setupTableName()
    {
        if (empty($this->_name) && basename(($this->_name = session_save_path())) != $this->_name) {
            ...
            throw new Indi_Session_SaveHandler_Exception('session.save_path is a path and not a table name.');
        }
    ...
    /**
     * Retrieve session lifetime considering Indi_Session_SaveHandler_DbTable::OVERRIDE_LIFETIME
     *
     * @param Indi_Db_Table_Row_Abstract $row
     * @return int
     */
    protected function _getLifetime(Indi_Db_Table_Row_Abstract $row)
    {
        $return = $this->_lifetime;
     ...
     * @return int
     */
    protected function _getExpirationTime(Indi_Db_Table_Row_Abstract $row)
    {
